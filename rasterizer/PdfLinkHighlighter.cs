...
namespace PdfDocumentLinkHighlighter
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // open PDF document
            using (FileStream fs = new FileStream("../../files/test.pdf", FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {               
                using (Document doc = new Document(fs))
                {
                    ...
                    foreach (Link link in firstPage.Links)
                    {
                        if (link.IsUriLink)
                        {
                            Apitron.PDF.Rasterizer.Rectangle locationRect = link.GetLocationRectangle(renderingResolution, renderingSettings);
            ...
            using (SearchIndex search = new SearchIndex(new FileStream("../../files/test.pdf", FileMode.Open, FileAccess.Read, FileShare.ReadWrite)))
            {
                search.Search(handlerArgs =>
                {
                    ...
                    {
                        SearchResultRegion searchResultRegion = firstPage.TransformRegion(item.Region, renderingResolution, renderingSettings);
        ...
        {
            return new RectangleF((float)locationRect.Left, (float)(height - locationRect.Top), (float)locationRect.Width, (float)locationRect.Height);
        }
    }
}